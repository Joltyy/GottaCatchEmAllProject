...
Enemy::Enemy(std::string img, float x, float y, float radius, float speed, float hp, int money, int exp, float damage, float box) :
	Engine::Sprite(img, x, y), speed(speed), hp(hp), money(money) , exp(exp), damage(damage){
